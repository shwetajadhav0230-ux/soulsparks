        ...
        {loading ? (
          ...
          filteredItems.map(item => (
            <motion.div 
              ...
              className="bg-white/90 backdrop-blur-md rounded-[2.5rem] p-6 shadow-sm border border-stone-100 group cursor-pointer hover:shadow-2xl transition-all"
            >
              <div className="relative overflow-hidden rounded-[2rem] mb-6 aspect-video shadow-inner bg-stone-200">
                ...
                <div className="absolute inset-0 bg-stone-900/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all backdrop-blur-[2px]">
                  <div className="bg-white/20 backdrop-blur-md p-4 rounded-full scale-90 group-hover:scale-100 transition-transform"><PlayCircle className="text-white w-12 h-12 drop-shadow-lg" /></div>
                </div>
              </div>
              ...
              <h3 className="font-serif text-xl text-stone-900 mb-2 font-medium leading-tight">{item.title}</h3>
              <p className="text-stone-400 text-sm italic">Guide: {item.instructor || 'SoulSpark AI'}</p>
            </motion.div>
          ))
        ) : (
          <div className="col-span-full text-center py-20 text-stone-400 italic">No videos found for this category yet.</div>
        )}
      </motion.div>
      ...
      <AnimatePresence>
        {selectedVideo && (
          ...
          >
            ...
            >
              <button onClick={() => setSelectedVideo(null)} className="absolute top-6 right-6 z-20 p-2 bg-black/50 hover:bg-black/80 rounded-full text-white backdrop-blur-md transition-colors">
                <X className="w-6 h-6" />
              </button>
              <iframe 
